...
"""
NCBI SCORING & CLASSIFICATION LOGIC
-----------------------------------
This module implements the core mathematical and heuristic logic for the NCBI Rescue phase.

It includes:
1. Regex patterns for parsing biological names (Candidatus, infraspecific ranks).
2. Name normalization utilities.
3. Scoring algorithms to compare candidate models against NCBI name pools.
4. Confidence computation (Tiering) based on score distributions.
5. Reclassification logic (Green/Yellow/Red) based on strict biological matching rules.
"""
...
# Matches rank marker + epithet pair (e.g., "subsp. lactis")
...
def _soft_rescue_stage(n_top_ties: int, top_score: float, median_score: Optional[float],
                       tie_red_threshold: int = 5, median_yellow_threshold: float = 70.0) -> Optional[str]:
    """
    Determines if a 'yellow' tier classification is warranted based on softened tie conditions.
    
    Used when specific Genus-Species equality is absent, but the score distribution 
    suggests a likely correct match.

    Conditions:
      - top_score must be exactly 100.
      - Number of ties must be low (<= tie_red_threshold).
      - Median score of non-top candidates must be low (indicating high specificity).

    Returns:
        'yellow' if conditions are met, otherwise None.
    """
    try:
...
def _extract_infraspecific_epithet(name: str) -> Optional[str]:
    """Extracts the epithet following an infraspecific rank marker."""
...
def _sanitize_punct(s: str) -> str:
    """Removes distinct punctuation characters and normalizes whitespace."""
    s = re.sub(r"[\[\]\(\)\{\}:;,\|]+", " ", s)
...
def _strip_candidatus(s: str) -> str:
    """Removes 'Candidatus' prefix."""
    return _CANDIDATUS_RE.sub("", s).strip()


def _strip_infraspecific(s: str) -> str:
    """Truncates the string at the first occurrence of an infraspecific marker."""
    s2 = s
...
def normalize_name(raw: str) -> Tuple[str, Dict[str, Any]]:
    """
    Standardizes a raw taxonomic name for database querying.
    
    Removes 'NAME::' prefixes, authorship information, and trailing 'sp.'.
    Returns the cleaned name and a metadata dictionary detailing the changes.
    """
    meta = {"original": raw}
    name = raw or ""
...
def _norm(s: str) -> str:
    """Basic string normalization: trimming and bracket handling."""
    if not s:
        return ""
    s = re.sub(r"\s+", " ", s.strip())
    s = re.sub(r"^\[([^\]]+)\]\s*", r"\1 ", s)
...
def _canon_name(s: str) -> str:
    """Produces a canonical form for comparison (lowercase, no special chars, 'sp' normalized)."""
    if not s:
        return ""
    s = s.replace("/", " ")
    s = re.sub(r"[_\-.]", " ", s)
...
def _is_sp_placeholder(s: str) -> bool:
    """Checks if the name represents a generic placeholder (e.g., 'Clostridium sp')."""
    s2 = _canon_name(s)
    return bool(re.search(r"^\w+\s+sp(\s|$)", s2))


def _norm_tokens(s: str) -> List[str]:
    """Splits a string into normalized tokens based on common separators."""
    s = re.sub(r"[=:\-\./\+\(\)]", "_", s)
    s = re.sub(r"_+", "_", s)
    s = s.strip("_")
...
def _extract_gs_from_model_name(mat: str) -> Optional[str]:
    """Extracts the first two tokens (Genus Species) from a model filename."""
...
def score_gem_against_pool(model_id: str, pool: set[str], lineage_genera: list[str], info: dict = None) -> Tuple[int, str]:
    """
    Scores a candidate model against the pool of valid names for a taxon.

    Scoring Tiers:
    - 100: Exact Genus+Species match.
    - 75:  Genus + 'sp' placeholder match.
    - 70:  Genus matches NCBI lineage AND NCBI canonical is a placeholder.
    - 60:  Genus matches NCBI lineage.
    - 20:  Weak match / Partial token overlap.
...
def compute_confidence_from_scores(scores: List[Any], tie_red_threshold: int = 5, median_yellow_threshold: float = 75.0) -> Dict[str, Any]:
    """
    Aggregates a list of candidate scores into a single confidence tier (Green, Yellow, Red).

    Logic:
    - Green: Exactly one candidate with the top score (unambiguous).
    - Red:  Top score < 100 OR too many ties at the top.
    - Yellow: Ambiguous but specific (low median score among non-winners).
    """
    s = []
    if scores:
        try:
...
# ==============================================================================
# 5. NCBI RECORD PARSING
# ==============================================================================

def parse_tax_record(rec: Dict[str, Any]) -> Dict[str, Any]:
    """Parses raw JSON record from NCBI cache into a structured dictionary."""
...
def cache_fetch_by_taxids(taxids: List[str], cache: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Retrieves multiple records from cache by TaxID."""
    if not taxids:
        return []
...
def expand_with_aka_merged(base: Dict[str, Any], cache: Dict[str, Any]) -> Dict[str, Any]:
    """Expands synonyms by looking up merged or AKA TaxIDs in the cache."""
    extra_ids = []
...
def build_name_pool(info: Dict[str, Any]) -> Set[str]:
    """Builds a comprehensive set of valid names (Synonyms + Lineage permutations)."""
    ...
    # Generate combinations of Lineage Genera + Specific Epithet
...
def _canonical_to_gs(canonical: str) -> str:
    """Converts a canonical name to 'Genus Species' string."""
    if not canonical:
        return ""
...
def _gs_aliases_from_pool(synonyms_field: str) -> set[tuple[str, str]]:
    """Parses synonym string into a set of (Genus, Species) tuples."""
    aliases = set()
...
# ==============================================================================
# 6. RECLASSIFICATION LOGIC (STRICT CHECK)
# ==============================================================================

def norm_id_strict(s: str) -> str:
    """Strict normalization for ID comparison (underscores, lowercase)."""
    s = re.sub(r"[^\w]+", "_", s)
...
def _classify_match_strict(input_name: str,
    ...
    """
    Performs a strict token-based comparison to determine match level.
    Returns: 'full_exact', 'gs_exact', or 'none'.
    """
...
def _reclassify_row(row: pd.Series) -> pd.Series:
    """
    Applies strict biological rules to reclassify a row's color/confidence.
    
    Can upgrade a match to Green (full_exact) or downgrade to Red/Yellow
    based on tie counts and exact token matching.
    """
    ...
    # Species Elevation Logic:
    # If input is "Genus species subsp. epithet" and model is "Genus epithet",
    # treat it as a full exact match (elevation of subspecies to species).
    ...
    # Default fallback for matches that have genus-species equality but failed stricter checks
    return pd.Series({
        "final_color": "yellow",
        "match_level": "gs_exact",
